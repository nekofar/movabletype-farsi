...
sub g2jstrftime($$) # just Like strftime = ( @_[0] = pattern, @_[1] = timestamp )
{
  ...
  my $t_s = 'قֽظֽ';
  $t_s = 'بֽظֽ' if ( $hour_24 >= 12 );
  my ($j_y, $j_m, $j_d) = gregorian_to_jalali ($g_y, $g_m, $g_d);

  my ($j_d_s, $j_d_i, $j_d_w);
  if ( $g_d_s eq 'Sat' ) { ($j_d_s, $j_d_i) = ('شنبه', 'شنبه'); $j_d_w = '0'; }
  elsif ( $g_d_s eq 'Sun' ) { ($j_d_s, $j_d_i) = ('یکشنبه', '۱ شنبه'); $j_d_w = '1'; }
  elsif ( $g_d_s eq 'Mon' ) { ($j_d_s, $j_d_i) = ('دوشنبه', '۲ شنبه'); $j_d_w = '2'; }
  elsif ( $g_d_s eq 'Tue' ) { ($j_d_s, $j_d_i) = ('سه شنبه', '۳ شنبه'); $j_d_w = '3'; }
  elsif ( $g_d_s eq 'Wed' ) { ($j_d_s, $j_d_i) = ('چهارشنبه', '۴ شنبه'); $j_d_w = '4'; }
  elsif ( $g_d_s eq 'Thu' ) { ($j_d_s, $j_d_i) = ('پنجشنبه', '۵ شنبه'); $j_d_w = '5'; }
